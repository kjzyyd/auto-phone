...
- **网页版豆包**：Playwright 打开 `doubao.com/chat`，扫码登录一次，登录态长期保存。
  - **可选：改用你本机的 Edge 浏览器**（保留登录态，不必额外扫码）。
...
默认用软件内置的 Chromium（随 exe 打包，登录态单独保存）。若你已在本机 Edge 登录豆包，可改为直接调用 Edge：

1. 「设置 → 浏览器来源」选 **本机 Edge 浏览器（保留登录态）**。
2. 点「连接豆包」：软件会自动启动一个带调试端口的 Edge 窗口（独立数据目录，**不影响**你日常用的 Edge），并在里面打开豆包网页。
3. 首次在窗口里扫码登录一次，登录态会保存在 Edge 专用目录（`edge_profile`），下次免登录。

> 说明：Edge 用独立数据目录是为了保证调试端口稳定可用；与日常 Edge 的账号、书签互不影响。
> 若你已自己启动带调试端口的 Edge（例如 `msedge --remote-debugging-port=9222`），软件会直接连接它。
